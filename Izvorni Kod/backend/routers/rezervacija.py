...
async def rezervacija_create(rezervacija: RezervacijaModel, db: db_dependency):
    ...
    vecNapravljene = db.query(models.Rezervacija).filter(models.Rezervacija.mbo == rezervacija.mbo).filter(models.Rezervacija.vrijemePoc > (date.today() + timedelta(days=1))).first()
